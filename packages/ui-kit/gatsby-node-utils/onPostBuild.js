...
exports.getOnPostBuild = dir => () => {
  const prefix =
    process.env.DEPLOY_TO_PREFIX || process.env.INPUT_DEPLOY_TO_PREFIX || ''
  ...
  } else if (process.env.INPUT_DEPLOY_TO) {
    // used for zeit now deployment
    fs.renameSync(
      path.join(dir, 'public'),
      path.join(dir, prefix + process.env.INPUT_DEPLOY_TO)
    )
  ...
  } else if (process.env.INPUT_DEPLOY_MOVE_TO) {
    // used for netlify deployment
    fs.moveSync(
      path.join(dir, 'public'),
      path.join(dir, process.env.INPUT_DEPLOY_MOVE_TO),
      { overwrite: true }
    )
  }
}
